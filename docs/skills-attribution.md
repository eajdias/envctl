...
`agent-memory`, `api-contract-design`, `ask-questions-if-underspecified`, `aur-headless-install`, `bulk-postgres-import`, `cachyos-gaming-setup`, `context7-auto`, `database-ops`, `docker`, `docker-build-local-vps-deploy`, `docker-desktop-wsl-restart`, `git-workflow`, `headless-gui-probe`, `jwt-hs256-node`, `lsp-smoke-test`, `memory-promotion`, `nextjs-standalone-deploy`, `parallel-agent-orchestration`, `phone-e164-normalization`, `playwright-prod-regression`, `simple-feature-flag`, `ssh-vps`, `subagent-routing`, `universal-test-runner`, `variant-analysis`, `vps-agent-dispatch`, `vps-provisioning`, `web-dashboard-automation`, `windows-admin`
