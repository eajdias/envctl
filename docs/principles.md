...
- O binário compilado (`envctl` / `envctl.exe`) carrega todos os manifestos declarativos, templates de configuração e as 38 Skills em tempo de compilação.
...
5. **Ecossistema OpenCode & CommandCode com 38 skills:**
   - Implantação de `opencode.json`, `dcp.jsonc`, `package.json` e `AGENTS.md`.
   - Extração das 38 Skills do OpenCode/CommandCode para `~/.config/opencode/skills` e `~/.commandcode/skills`.
