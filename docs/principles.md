...
- O binário compilado (`envctl` / `envctl.exe`) carrega todos os manifestos declarativos, templates de configuração e as 43 Skills em tempo de compilação.
...
4. **Toolchains & Language Servers (LSPs):**
   ...
   - Python: `uv`, `ruff`.
   - Go: `gopls`.
5. **Ecossistema OpenCode & CommandCode com 43 skills:**
   - Implantação de `opencode.json`, `package.json` e `AGENTS.md`.
   - Extração das 43 Skills do OpenCode/CommandCode para `~/.config/opencode/skills` e `~/.commandcode/skills`.
