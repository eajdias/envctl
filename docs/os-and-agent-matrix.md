...
| Validação de agente no doctor | **Config shape**: raiz `agent`/`permission`, mode, `description` em dispatchable, actions `bash`/`task`, campos legacy V1 por agente | **Agents**: `name == filename`, nome não reservado, description não vazia, schema do frontmatter (`tools`/`model`/`reasoningEffort`/`maxTurns`/`permissionMode`/`background`/`showOutput`) |
...
| 16 | opencode v2 ignora runtime LSP (`lsp` aceito mas inerte) e o `subagent_depth` **top-level** (WARN `omitted unsupported legacy setting`) | **Resolvido 2026-09-22** — bloco `lsp` removido dos dois configs (formato nativo V2: `agents`/`permissions[]`/`plugins`/`skills[]`/`mcp.servers`); `review` com `mode: primary` explícito, `plan` sem `mode` (preserva o built-in — customs só com IDs novos); binários LSP seguem provisionados p/ shell/IDE e o `doctor` os audita como toolchain. **Correção 2026-09-26**: o texto original afirmava também que `subagent_depth` e `instructions` tinham sido removidos, e isso nunca foi verdade — `experimental.subagent_depth` (chave aninhada, não a top-level) e `instructions` continuam nos dois configs porque o runtime V2 **os lê** (`Config.Instructions` + leitura pelo subagent tool, conferido no binário v2.0.8 e registrado em `configs/memory/patterns.md`). Só o `lsp` saiu |
...
| 24 | O OpenCode não tinha **nenhum reviewer dispatchável**: `review` é `mode: primary` e, por isso, não aparece no catálogo de subagentes (mesma semântica que excluiu o `plan` na #18), os built-ins V2 são só `build`/`plan`/`general`/`explore`, e o CommandCode tinha `code-reviewer` dispatchável — o orquestrador no OpenCode não conseguia pedir um segundo parecer read-only | **Resolvido 2026-09-26** — novo custom `reviewer` com `mode: subagent` nos dois configs (ID novo, `description` com gatilhos, `edit * deny` + `subagent * deny`, mesmo contrato de severidades BLOCKER/MAJOR/MINOR/NIT e linha `VERDICT` do `review`); a skill `subagent-routing` ganhou a linha de review nomeando **os dois** nomes por runtime, e `TestSubagentRoutingNamesEveryDispatchableAgent` trava a lista |
| 25 | `validateOpenCodeConfigShape` só checava a raiz `agent`/`permission` e as actions `bash`/`task`: um agente escrito no estilo V1 (`"permission": {"edit": "deny"}`, `"prompt"`, `"tools"`, `"temperature"`, `"top_p"`, `"disable"`, `"maxSteps`) é **ignorado em silêncio** pelo V2 — o agente carrega com o prompt base do provider e com as permissões default, ou seja, um "read-only" que edita freely. A doc V2 lista esses campos como legacy explicitamente | **Resolvido 2026-09-26** — `legacyAgentFields` (campo → substituto V2) entra no check `Config shape` como WARN por agente, com ordem estável; `request.body.temperature`/`top_p` (a forma V2 válida) e `disabled`/`hidden` **não** disparam, provado por caso no teste. Modo `all` também exige `description`, como `subagent` |
| 26 | Os templates de agente do CommandCode (`configs/commandcode/agents/*.md`) não tinham **nenhum teste**: o `doctor` só enxerga o que já está provisionado na máquina, então um template quebrado (nome ≠ filename, tool inexistente, `agent` concedido, description vazia, nome reservado) chegava intacto a todo mundo e só falharia depois do `envctl commandcode` — o lado OpenCode já tinha `TestShippedOpenCodeTemplatesHaveNativeShape` | **Resolvido 2026-09-26** — `TestShippedCommandCodeAgentTemplatesMatchSchema` valida cada template embutido contra `validateCommandCodeAgentFrontmatter` (WARN **e** INFO reprovam, porque quem manda é o repo), exige `name == filename`, `description` não vazia e nome fora do conjunto reservado (`explore`/`plan`/`review`/`general`, extraído para `commandCodeReservedAgentNames` e reaproveitado pelo check `Agents` do doctor), e fixa a lista de agentes embarcados. Guard provado por 5 mutações: nome≠filename, nome reservado, tool fora do catálogo, `agent`/`agent_output` concedidos e description vazia |
...
4. `description` com **gatilhos** no fim ("Triggers: ..."): é ela que o modelo lê para escolher o subagente, e o catálogo do runtime é truncado — descrição curta e densa, como as de `planner`/`reviewer`.
5. Nenhum campo legacy V1 por agente (`prompt`, `permission`, `tools`, `temperature`, `top_p`, `disable`, `maxSteps`): o V2 ignora em silêncio. `request.body.temperature` é a forma válida. O check `Config shape` reprova todos eles (assimetria #25).
6. `permissions[]` é lista ordenada e **a última regra que casa vence**: abra `*` antes das exceções. `edit * deny` + `edit <escopo> allow` é o padrão de escrita restrita (`planner` → `spec-agent/**`, `docs-writer` → `docs/**` + `README.md` com `CHANGELOG.md` em deny explícito, `memory-keeper` → `**/memory/**`).
7. Subagente que despacha subagente leva `subagent * deny`: a delegação tem um nível por convenção da família read-only, para o subagente não estourar o contexto do coordenador em cascata.
