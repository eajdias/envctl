...
5. `StartupItem` sonda e remove contra um conjunto fechado (2 Run keys + 2 pastas `Startup`), **não** via `Win32_StartupCommand`: essa classe é uma `CIM_Setting` sem método `Delete` (o MOF lista só properties) e o `Location` dela é inconsistente — um classificador sobre ele casava com a forma errada e o doctor ficava verde sem remover nada. `startupTargetKind()` roteia cada local para o cmdlet certo e check/batch/apply passam todos por `probeStartup`. Serviços de áudio (`WavesSvc`, `RtkAuduService`) ficaram de fora de propósito.
