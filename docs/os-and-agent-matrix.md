...
5. `StartupItem` sonda e remove contra um conjunto fechado (2 Run keys + 2 pastas `Startup`), **não** via `Win32_StartupCommand`: essa classe é uma `CIM_Setting` sem método `Delete` (o MOF lista só properties) e o `Location` dela é inconsistente — um classificador sobre ele casava com a forma errada e o doctor ficava verde sem remover nada. A sonda emite um **token** por alvo (nunca um path: code page do console corrompe path não ASCII, e `;` é legal em nome de pasta), `startupTargetForToken()` recusa token desconhecido, o nome da Run key é escapado com `[WildcardPattern]::Escape()` (pois `-Name` do provider de registro é sempre wildcard), e uma sonda parcial vira erro em vez de "ausente". Check/batch/apply passam todos por `probeStartup`. Serviços de áudio (`WavesSvc`, `RtkAuduService`) ficaram de fora de propósito.
