...
ssh homologacao-chatbot 'opencode run "Diagnosticar uso de memória dos containers Docker e retornar tabela formatada" < /dev/null'

# 2. Execução desacoplada em background (tarefas longas)
ssh homologacao-chatbot 'nohup opencode run "Executar suíte de testes de integração e gerar relatório em /tmp/report.md" < /dev/null > /tmp/agent.log 2>&1 &'

# 3. Coleta do resultado consolidado
ssh homologacao-chatbot 'cat /tmp/report.md'
```
