...
- **`subagent-supervision`**: O coordenador vigia subagentes paralelos — monitora progresso e, se um alucina/loopa/trava, interrompe a sessão do OpenCode V2 via `opencode api post /api/session/<sessionID>/interrupt` (o runtime **não** expõe `agent_output`/`agent_id`; `sessionID` ≠ PID) e decide retry (uma única tentativa, com prompt refinado) ou escala ao usuário. Nunca retry infinito, nunca `reset`/`clean` no worktree.
- **`task-hang-watchdog`**: Previne e recupera terminais travados e tarefas autônomas presas — timeout real, comandos não interativos, saída persistente em log e recuperação que preserva worktree/logs; encerramento suave antes de forçado, sempre com PID rastreado (nomes de tools são específicos do runtime ativo).
