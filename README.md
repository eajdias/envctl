...
- **Ecossistema OpenCode & CommandCode com 50 Skills**: `opencode.json`, plugins e **50 Skills de Agentes de IA provisionadas** (+ 1 built-in do opencode). Suporte equivalente a **CommandCode** (agente `code-reviewer`, MCPs, configs) — diferenças de plataforma documentadas na [tabela de paridade](docs/skills.md).
...
envctl run skills       # Extração e sincronização das 50 Skills
...
- 🤖 [**Catálogo de Skills & Subagentes**](docs/skills.md) — As 50 Skills provisionadas, roteamento de subagentes, orquestração remota (`vps-agent-dispatch`) e automação de browser via MCP.
...
1. **100% Standalone via `//go:embed`**: Todas as 50 Skills e templates residem dentro do próprio binário executável compilado.
