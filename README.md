...
- **Ecossistema OpenCode & CommandCode com 40 Skills**: `opencode.json`, `dcp.jsonc`, plugins e **40 Skills de Agentes de IA provisionadas** (+ 1 built-in do opencode). Suporte equivalente a **CommandCode** (agente `code-reviewer`, MCPs, configs) — diferenças de plataforma documentadas na [tabela de paridade](docs/skills.md).
...
envctl run skills       # Extração e sincronização das 40 Skills
...
- 🤖 [**Catálogo de Skills & Subagentes**](docs/skills.md) — As 40 skills provisionadas, roteamento de subagentes, orquestração remota (`vps-agent-dispatch`) e automação de browser via MCP.
...
1. **100% Standalone via `//go:embed`**: Todas as 40 Skills e templates residem dentro do próprio binário executável compilado.
