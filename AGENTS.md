...
12 agent skills (OpenCode + CommandCode), 15 LSPs, Windows tweaks, and a local
...
- **Agents:** `planner`, `reviewer`, `verifier`, `docs-writer` and `memory-keeper` (dispatchable read-only subagents) for bulky research/plans, an independent second review, gate evidence, doc drift and the memory wrap-up; the `plan` Tab for interactive planning, `review` on the Tab before concluding — see `docs/os-and-agent-matrix.md` §2/§3. In CommandCode there is no `planner`: its built-in `plan` is already dispatchable and read-only, and the coordinator materializes the spec. There the same roles ship as agent files, and the reviewer is named `code-reviewer` because `review` is a reserved runtime name.
