...
3. `internal/usecase/doctor_audit.go:1208-1211,1222` — (a) crash-with-no-output (`cmdErr != nil && len(out)==0`) currently `continue`s SILENTLY (the most-broken server is the quietest): emit `DiagWarning` naming `lsp.ServerName` + `CheckBinary` ("crashed on spawn with no output") with FixHint `envctl run lsp`. (b) `repro` string (line 1222) hardcodes `< /dev/null`: build it per-platform — `" < NUL"` when `runtime.GOOS=="windows"` else `" < /dev/null"` (verify `runtime` is imported in `doctor_audit.go`; `os` already is — line 1199 uses `os.DevNull`, which is ALREADY correct at runtime, only the displayed string is wrong). (c) Keep the sequential 10s×N loop (worst case ~140s for 14 LSPs) — parallelization is explicitly OUT of scope; note it in the commit message as future work.

**Acceptance.** `go build ./... && ./envctl run badname 2>&1 | grep providers` (lists it); `ls -l ~/.envctl/logs/envctl-*.log | awk '{print $1}'` (want `-rw-------` on new logs); LSP handshake warning names the binary.
