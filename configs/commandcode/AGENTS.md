...
- **Git:** `fscache`, `preloadindex`, `longpaths`, `autocrlf=input`, pager `delta`.  ·  **Worktrees:** o runtime gerencia em `~/.commandcode/worktrees/<repo>-<hash>/` (fora do repo) via `/worktree` e `enter_worktree`; para cair na convenção do projeto use `cmdc -w "$PWD/.worktrees/<slug>"` (path absoluto é usado verbatim). `doctor` reporta `prunable`/`locked` nas duas rotas.
