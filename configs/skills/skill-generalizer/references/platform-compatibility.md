...
| Command Code | Native Agent Skills (agentskills.io standard). Discovers `.commandcode/skills/`, `~/.commandcode/skills/`, plus `.agents/skills/` compatibility paths. Frontmatter is tolerant of other agents' fields (`argument-hint`, `disable-model-invocation`, `allowed-tools`, `metadata`) and ignores unknown ones. Invoked as `/<name>` or `/skill:<name>`. | `.commandcode/skills/` for project skills, `~/.commandcode/skills/` for global skills, `.agents/skills/` for portable repo skills. |
