...
## Quando usar
Após `envctl run gaming`, ou para auditar tuning existente. O envctl provisiona
pacotes (`gaming.yaml`) e presets de usuário (`gaming.conf`, `MangoHud.conf`);
tudo que exige root, reboot ou decisão de segurança vive SÓ nesta skill
(como orientação manual) e no `doctor` (como auditoria read-only, nunca `--fix`).
