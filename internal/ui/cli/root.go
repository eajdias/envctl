...
func InitApp(embeddedFS fs.FS, version string) {
	...
	appCtx = &AppContext{
		...
		ProvisionPerformanceUC: usecase.NewProvisionPerformanceUseCase(manifestRepo, packagesUC, sysctlManager, zramManager, fileLogger, entity.DetectedPlatform, performance.NewTimezoneManager(), performance.NewJournaldManager()),
