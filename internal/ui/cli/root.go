...
func InitApp(embeddedFS fs.FS) {
	...
	appCtx = &AppContext{
		...
		ProvisionPkgsUC:   usecase.NewProvisionPackagesUseCase(manifestRepo, pkgManagers),
		ProvisionShellUC:  usecase.NewProvisionShellUseCase(manifestRepo, fsManager, envManager, gitManager, embeddedFS),
		ProvisionSkillsUC: usecase.NewProvisionSkillsUseCase(manifestRepo, fsManager, embeddedFS),
		ProvisionLSPUC:    usecase.NewProvisionLSPsUseCase(manifestRepo, pkgManagers),
		DoctorAuditUC:     usecase.NewDoctorAuditUseCase(manifestRepo, fsManager, envManager, gitManager, pkgManagers),
		SnapshotSyncUC:    usecase.NewSnapshotSyncUseCase(manifestRepo, fsManager, gitManager),
	}
